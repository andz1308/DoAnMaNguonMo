                    ...
                    @foreach($cart->chiTietDonHang as $item)
                        @php
                            ...
                            if ($isOutOfStock) {
                                ...
                                $totalMoney += $thanhTien; // Cộng tiền đã giảm giá
                            }
                        ...
                        <tr class="{{ $isOutOfStock ? 'row-stock-error' : '' }}">
                            ...
                            <td>
                                ...
                                @else
                                    <div class="product-meta"><i class="icon-tag"></i> Màu sắc: Đen</div>
                                @endif
                            </td>
                            <td>
                                <form action="{{ route('cart.update', $item->id) }}" method="POST" class="quantity-form">
                                    ...
                                    <a href="{{ route('cart.remove', $item->id) }}" class="btn btn-danger btn-small btn-remove-cart"
                                        title="Xóa" onclick="return confirm('Bạn có chắc muốn xóa sản phẩm này?')">
                            ...
                            <td>
                                @if($sanPham->gia != $sanPham->gia_ban)
                                    <!-- Có khuyến mãi: Hiện giá cũ gạch ngang + Giá mới -->
                                ...
                                @else
                                    <!-- Không khuyến mãi: Hiện giá gốc -->
                                    <span>{{ number_format($sanPham->gia, 0, ',', '.') }} ₫</span>
                                @endif
                            </td>
                            <!-- Thành tiền cũng hiển thị theo giá đã giảm -->
